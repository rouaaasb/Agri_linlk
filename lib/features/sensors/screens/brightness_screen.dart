...
class _BrightnessScreenState extends State<BrightnessScreen> {
  final DatabaseReference _sensorRef = FirebaseDatabase.instance.ref('sensors/brightness');
  ...
  void initState() {
    super.initState();
    _listenToRealtimeBrightness();
  }

  void _listenToRealtimeBrightness() {
    _sensorRef.onValue.listen((event) {
      final data = event.snapshot.value as Map?;
      if (data != null) {
        final double? newBrightness = double.tryParse(data['value'].toString());
  ...
  String _getBrightnessComment(double? value) {
    if (value == null) return "";
    if (value > 800) {
      return "☀️ Very bright - Use shading to protect light-sensitive plants.";
    } else if (value > 600) {
      return "🌤️ Bright - Excellent for sun-loving plants.";
    } else if (value > 400) {
      return "⛅ Moderate - Sufficient for most plants, monitor those needing high light.";
    } else {
      return "🌥️ Low light - Consider using grow lights to support healthy growth.";
    }
  }
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            Text(
              brightness != null ? '${brightness!.toStringAsFixed(1)} lx' : '--',
            ...
            if (brightness != null)
              Text(
                _getBrightnessComment(brightness),
